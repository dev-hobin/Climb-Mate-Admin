...
const BannerImageUploadView = class extends View {
  ...
  _bindEvents = () => {
    ...
    this._imageInput.addEventListener('change', event => {
      ...
      this.trigger('@addImages', { fileList });
    });
    // 업로드 트리거 이벤트 달기
    this._uploadBtn.addEventListener('click', () => this.trigger('@uploadImages', {}));
  };
  ...
  _addItemEvent = item => {
    ...
    item.addEventListener('dragend', () => {
      ...
      if (this._dragStartIndex !== this._dragEndIndex)
        this.trigger('@changeImageLocation', { beforeIndex: this._dragStartIndex, afterIndex: this._dragEndIndex });
      // 드래그 끝내면 인덱스는 초기화
      this._dragStartIndex = null;
      this._dragEndIndex = null;
    });
